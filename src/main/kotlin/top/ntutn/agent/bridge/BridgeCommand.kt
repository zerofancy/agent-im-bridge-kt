...
data class BridgeCommandDefinition(val name: String, val arguments: String = "", val description: String)
...
data class BridgeCommand(val name: String, val argument: String) {
    companion object {
        ...
        fun help(backendName: String) = definitions(backendName).joinToString("\n") {
            "${it.name}${if (it.arguments.isEmpty()) "" else " " + it.arguments} — ${it.description}"
