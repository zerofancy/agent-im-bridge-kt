#!/usr/bin/env python3
"""Opt-in launchd integration: temporary root, fake app-server, no Feishu credentials or real models."""
...
def main():
    if sys.platform!='darwin': raise RuntimeError('macOS only')
    repo=Path(__file__).resolve().parent.parent
    root=Path(tempfile.mkdtemp(prefix='bridge-launchd-test-'))
    ...
    try:
        ...
        label=m.label+'.deploy.'+resumed
        def executor_pid():
            text=b.run(['launchctl','print',m.domain+'/'+label],check=False).stdout
            match=re.search(r'(?m)^\s*pid = (\d+)',text)
        ...
        # The loaded launchd arguments are still held: durable activation must also survive a later crash.
        ...
        assert b.run(['launchctl','print',m.domain+'/'+m.label],check=False).returncode!=0
        print('PASS manual stop stays stopped',flush=True)
        print('ALL LAUNCHD CHECKS PASSED',flush=True)
    finally:
        try:m.stop()
        except Exception:pass
        try:prod.stop()
        ...
        for manager in (m,prod):
            for p in (Path.home()/'Library/LaunchAgents').glob(manager.label+'*.plist'):
                b.run(['launchctl','bootout',manager.domain+'/'+p.stem],check=False)
                b.run(['launchctl','enable',manager.domain+'/'+p.stem],check=False)
